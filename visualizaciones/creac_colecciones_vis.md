...
```terminal
mongoexport --host localhost --db apilegislativo --collection sexenios_vis --type=csv --out sexenios_vis.csv --fields _id,count
...
mongoexport --host localhost --db apilegislativo --collection sexenios_des_vis --type=csv --out sexenios_des_vis.csv --fields _id,count
...
EXTRA, para las gráficas

EPN iniciativas aprobadas por trimestre

```javascript
db.iniciativasaprobadas.aggregate( { $addFields: { fecha: { $toDate: "$date_anounced" } } }, { $match: { fecha: { $lt: ISODate("2018-12-01T00:00:00") } } },
{$addFields: {conv_date: {$toDate: "$date_anounced"}}},
// substr del mes y año.
{$addFields: {month: {$substr: ["$conv_date", 5, 2]}}}, //mes
{$addFields: {year: {$substr: ["$conv_date", 0, 4]}}}, //año
...
db.iniciativasaprobadas.aggregate( { $addFields: { fecha: { $toDate: "$date_anounced" } } }, { $match: { fecha: { $gte: ISODate("2018-12-01T00:00:00") } } },
{$addFields: {conv_date: {$toDate: "$date_anounced"}}},
// substr del mes y año.
{$addFields: {month: {$substr: ["$conv_date", 5, 2]}}}, //mes
{$addFields: {year: {$substr: ["$conv_date", 0, 4]}}}, //año
...
20 leyes mas modificadas del último año de EPN y lo que va de AMLO

#modificaciones que no están en el respo
- pasar a minúsculas
...
```terminal
mongoexport --host localhost --db apilegislativo --collection trimestres_EPN --type=csv --out trimestres_EPN_vis.csv --fields _id,count

mongoexport --host localhost --db apilegislativo --collection trimestres_AMLO --type=csv --out trimestres_AMLO_vis.csv --fields _id,count

mongoexport --host localhost --db apilegislativo --collection leyes_EPN_vis --type=csv --out leyes_EPN_vis.csv --fields _id,count

mongoexport --host localhost --db apilegislativo --collection leyes_AMLO_vis --type=csv --out leyes_AMLO_vis.csv --fields _id,count
