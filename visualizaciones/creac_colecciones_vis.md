...
//desechadas
//EPN
db.iniciativasdesechadas.aggregate( { $addFields: { fecha: { $toDate: "$date_anounced" } } }, { $match: { fecha: { $lt: ISODate("2018-12-01T00:00:00") } } }, {$group: {_id: "sexenio_EPN", count: {$count: {}}}}, {$out:'sexenio_des_vis'});
//AMLO
db.iniciativasaprobadas.aggregate( { $addFields: { fecha: { $toDate: "$date_anounced" } } }, { $match: { fecha: { $gte: ISODate("2018-12-01T00:00:00") } } }, {$group: {_id: "sexenio_AMLO", count: {$count: {}}}}, {$merge: {into: "sexenio_des_vis"}});
```
